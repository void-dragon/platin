...
namespace {
  ...
  lib::sync::Mutex emergency_mutex;
}
...
__cxxabiv1::__cxa_allocate_exception( std::size_t thrown_size ) throw () {
  ...
  ret = lib::alloc( thrown_size );

  if ( !ret ) {
    emergency_mutex.enter();
  ...
  lib::memset( ret, 0, sizeof(__cxa_refcounted_exception ) );
...
extern "C" void __cxxabiv1::__cxa_free_exception( void *vptr ) throw () {
  ...
  if ( ptr >= base && ptr < base + sizeof( emergency_buffer ) ) {
    ...
    lib::free( ptr - sizeof(__cxa_refcounted_exception ) );
}
...
extern "C" __cxa_dependent_exception* __cxxabiv1:: __cxa_allocate_dependent_exception()        throw()
{
  ...
  (lib::alloc (sizeof (__cxa_dependent_exception)));
  ...
  lib::memset (ret, 0, sizeof (__cxa_dependent_exception));
