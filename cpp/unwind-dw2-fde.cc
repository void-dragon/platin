...
/* Copyright (C) 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2008,
 2009  lib::free Software Foundation, Inc.
 Contributed by Jason Merrill <jason@cygnus.com>.

 This file is part of GCC.

 GCC is lib::free software; you can redistribute it and/or modify it under
 the terms of the GNU General Public License as published by the lib::free
 ...
 3.1, as published by the lib::free Software Foundation.
...
static object *unseen_objects;
static object *seen_objects;

static lib::sync::Mutex object_mutex;
...
void __register_frame( void *begin ) {
  ...
  ob = ( object* ) lib::alloc( sizeof(struct object) );
...
void __register_frame_table( void *begin ) {
  object *ob = ( object* ) lib::alloc( sizeof(struct object) );
...
void __deregister_frame( void *begin ) {
  /* If .eh_frame is empty, we haven't registered.  */
  if ( *( uword * ) begin != 0 )
    lib::free( __deregister_frame_info( begin ) );
}
...
static int get_cie_encoding( const struct dwarf_cie *cie ) {
  ...
  p = aug + lib::strlen( ( const char * ) aug ) + 1; /* Skip the augmentation string.  */
...
static inline int get_fde_encoding( const struct dwarf_fde *f ) {
...
static int fde_unencoded_compare( struct object *ob __attribute__((unused)), const fde *x, const fde *y ) {
  ...
  lib::memcpy( &x_ptr, ( void* ) x->pc_begin, sizeof(_Unwind_Ptr) );
  lib::memcpy( &y_ptr, ( void* ) y->pc_begin, sizeof(_Unwind_Ptr) );
...
static int fde_mixed_encoding_compare( struct object *ob, const fde *x, const fde *y ) {
  ...
  read_encoded_value_with_base( x_encoding, base_from_object( x_encoding, ob ), x->pc_begin, &x_ptr );

  y_encoding = get_fde_encoding( y );
  read_encoded_value_with_base( y_encoding, base_from_object( y_encoding, ob ), y->pc_begin, &y_ptr );
...
static inline int start_fde_sort( struct fde_accumulator *accu, size_t count ) {
  size_t size;
  if ( !count )
    return 0;

  size = sizeof(struct fde_vector) + sizeof(const fde *) * count;
  if ( ( accu->linear = ( fde_vector* ) lib::alloc( size ) ) ) {
    accu->linear->count = 0;
    if ( ( accu->erratic = ( fde_vector* ) lib::alloc( size ) ) )
...
static inline void fde_insert( struct fde_accumulator *accu, const fde *this_fde ) {
  if ( accu->linear )
...
static inline void fde_split( struct object *ob, fde_compare_t fde_compare, struct fde_vector *linear, struct fde_vector *erratic ) {
  ...
  for ( i = 0; i < count; i++ ) {
    ...
    for ( probe = chain_end; probe != &marker && fde_compare( ob, linear->array[ i ], *probe ) < 0; probe = chain_end ) {
...
static inline void fde_merge( struct object *ob, fde_compare_t fde_compare, struct fde_vector *v1, struct fde_vector *v2 ) {
  size_t i1, i2;
  const fde * fde2;
...
static inline void end_fde_sort( struct object *ob, struct fde_accumulator *accu, size_t count ) {
  ...
  if ( accu->erratic ) {
    ...
    lib::free( accu->erratic );
  }
  else {
    /* We've not managed to lib::alloc an erratic array,
...
static size_t classify_object_over_fdes( struct object *ob, const fde *this_fde ) {
  ...
  for ( ; !last_fde( ob, this_fde ); this_fde = next_fde( this_fde ) ) {
    ...
    read_encoded_value_with_base( encoding, base, this_fde->pc_begin, &pc_begin );
...
static void add_fdes( struct object *ob, struct fde_accumulator *accu, const fde *this_fde ) {
  ...
  for ( ; !last_fde( ob, this_fde ); this_fde = next_fde( this_fde ) ) {
    ...
    if ( encoding == DW_EH_PE_absptr ) {
      _Unwind_Ptr ptr;
      lib::memcpy( &ptr, ( void* ) this_fde->pc_begin, sizeof(_Unwind_Ptr) );
      if ( ptr == 0 )
        continue;
    }
    else {
...
static inline void init_object( struct object* ob ) {
...
linear_search_fdes( struct object *ob, const fde *this_fde, void *pc ) {
  ...
  for ( ; !last_fde( ob, this_fde ); this_fde = next_fde( this_fde ) ) {
    ...
    if ( encoding == DW_EH_PE_absptr ) {
      ...
      p = read_encoded_value_with_base( encoding, base, this_fde->pc_begin, &pc_begin );
...
binary_search_unencoded_fdes( struct object *ob, void *pc ) {
  ...
  for ( lo = 0, hi = vec->count; lo < hi; ) {
    ...
    void *pc_begin;
    uaddr pc_range;
    lib::memcpy( &pc_begin, ( void* ) f->pc_begin, sizeof(void *) );
    lib::memcpy( &pc_range, ( void* ) ( ( const uaddr * ) f->pc_begin + 1 ), sizeof(uaddr) );
...
binary_search_single_encoding_fdes( struct object *ob, void *pc ) {
  ...
  for ( lo = 0, hi = vec->count; lo < hi; ) {
    ...
    p = read_encoded_value_with_base( encoding, base, f->pc_begin, &pc_begin );
    read_encoded_value_with_base( encoding & 0x0F, 0, p, &pc_range );
...
binary_search_mixed_encoding_fdes( struct object *ob, void *pc ) {
  ...
  for ( lo = 0, hi = vec->count; lo < hi; ) {
    ...
    p = read_encoded_value_with_base( encoding, base_from_object( encoding, ob ), f->pc_begin, &pc_begin );
    read_encoded_value_with_base( encoding & 0x0F, 0, p, &pc_range );
